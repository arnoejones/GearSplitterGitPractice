...
def diff_ratio_callback(diff_ratio_input_value):
    final_dict['differentialGearRatio'] = diff_ratio_input_value
...
def update_figure(n_clicks):
    ...
    for gear_index in range(0, len(gear_ratios)):
        df = pd.DataFrame(Calculate.JeepGearSplitter.calculateRpmFromSpeed(
            differentialGearRatio=final_dict['differentialGearRatio'],
